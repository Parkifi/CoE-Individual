...
//Rare exceptions to indicate a bug in the _meuarraial code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_meuarraial = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_meuarraial]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.meuarraial.module"
